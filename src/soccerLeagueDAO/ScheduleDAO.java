...
public class ScheduleDAO {
	...
	public static List<Schedule> listSchedule() {
		TypedQuery<Schedule> query = emDAO.getEM().createQuery("SELECT schedule FROM schedule schedule",
				Schedule.class);
		return query.getResultList();
	}

	public static Schedule findScheduleById(int id) {
		Schedule schedule = emDAO.getEM().find(Schedule.class, Integer.toString(id));
		return schedule;
	}
